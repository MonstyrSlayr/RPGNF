...
class OptionsMenu extends MusicBeatState
{
	...
	var options:Array<OptionCatagory> = [
		new OptionCatagory("Gimmicks", [
			new NoteInvertOption("Enable/Disable the note inversion that occurs in 'Blowaway-Hard'."),
		]),
		new OptionCatagory("Gameplay", [
			...
			// new OffsetMenu("Get a note offset based on your inputs!"),
		...
		new OptionCatagory("Misc", [
			...
			new TextEffectOption("Enable/Disable text effects, such as the wiggling happening on an option highlight.")
		])
	...
	override function create()
	{
		...
		for (i in 0...options.length)
		{
			...
			if (i == 0)
			{
				controlLabel.texeff = TextEffect.WAVY;
			}
			else
			{
	...
	function changeSelection(change:Int = 0)
	{
		...
		for (item in grpControls.members)
		{
			...
			item.alpha = 0.6;
			item.texeff = TextEffect.NORMAL;
			// item.setGraphicSize(Std.int(item.width * 0.8));

			if (item.targetY == 0)
			{
				item.alpha = 1;
				item.texeff = TextEffect.WAVY;
